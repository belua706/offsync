...
mod tests {
    ...
    fn create_temp_dir() -> TempDir {
        ...
        let mut new_file = NamedTempFile::new_in(&new_dir).unwrap();
        writeln!(&mut new_file, "test content 1").unwrap();
        let mut new_file = NamedTempFile::new_in(&new_dir).unwrap();
        writeln!(&mut new_file, "test content 2").unwrap();
    ...
    fn incorrect_sync(src: &Path, target: &Path) -> Result<(), SyncError> {
        sync(src, target);
        todo!("add some changes so that the sync is def not correct");
        Ok(())
    }
